...
def es_prevencion(user):
    return "prevencion" in user.username.lower()

@user_passes_test(es_prevencion)
def lista_autorizaciones_prevencion(request):
    autorizaciones = Autorizacion.objects.all().order_by('-id')
    paginator = Paginator(autorizaciones, 10)
    page_number = request.GET.get('page')
    page_obj = paginator.get_page(page_number)

    return render(request, 'prevencion/lista_autorizaciones.html', {
        'page_obj': page_obj
    })
